...
function RegisterAge() {
    ...
    useEffect(() => {
        birth.length === 8 ? setIsButton(false) : setIsButton(true);
    }, [birth])
    ...
    const onClick = () => {
        ...
        if (regBirth.test(birth)) {
            if (Number(birth.slice(0, 4)) < 2010) {
    ...
    return (
        <Frame>
            <Layout>
                <form>
                    <div>생년월일 입력</div>
                    <input type="text" onChange={onChange} placeholder="생년월일 8자리" value={birth} maxLength={Number(8)} />
                    <div>생년월일 정보는 가입 연령 확인에만 이용됩니다.</div>
                    <button type="button" onClick={onClick} disabled={isButton}>다음</button>
